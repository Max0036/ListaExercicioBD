...
from estudante as e join cursa ON e.matricula = matricula_est join disciplina as d ON d.codigo = cod_disc
...
for each row
begin
		insert into  log_funcionario(cpf_func,comando,usuario,data_hora) values (cpf_func,'UPDATE', user(),current_date());
        
END$
delimiter ;

update funcionario set nome = 'Juao' where cpf= '0987654321' ;

select * from log_funcionario;


DELIMITER $
create trigger tg_funcionario_Insert before insert on funcionario
for each row
begin
		insert into  log_funcionario(cpf_func,comando,usuario,data_hora) values (cpf_func,'INSERT', user(),current_date());
        
END$
delimiter ;

INSERT INTO funcionario VALUES ('1232123441', 'Auxiliar Adm', 'Joaquina', 'Rua das dores', '(41)2134-4567', '1974-01-01', '2013-09-05', 1000);
