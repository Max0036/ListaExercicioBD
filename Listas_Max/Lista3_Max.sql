...
delimiter $
create procedure sp_ClienteCadastrados () 
begin
select * from  Cliente;
end;
$

CALL atividade3.sp_ClienteCadastrados();

delimiter $
create procedure  sp_Produtos ( valor DOUBLE )
begin
select * from produto where precounitario < valor;
end;
$
...
delimiter $
create procedure sp_TotalVendas ( codigo_produto INT )
begin
select sum (quantidade) * sum (precounitario) from itemdopedido 
where codigoproduto = codigo_produto;
end;
$

CALL  sp_TotalVendas (77);

delimiter $
create procedure sp_Registro (v_codigoproduto integer,v_unidade character(3),v_descricaoproduto varchar(30), v_precounitario real )
begin
	if (( v_codigoproduto !='') && (v_unidade != '') && (v_descricaoproduto != '') && (v_precounitario != '')) then
    insert into produto (codigoproduto ,unidade ,descricaoproduto ,precounitario ) values (v_codigoproduto ,v_unidade ,v_descricaoproduto ,v_precounitario);
    else 
    select 'ERRO ! OS VALORES DEVEM SER FORNECIDOS PARA CADASTRAR OS PRODUTOS' as mensagem;
    end if;
    end; $ 
    
CALL sp_Registro ('10','S','Arroz','5.0');
CALL sp_Registro ('98','S','arroz','5.0');
CALL sp_Registro ('','','','') ;

delimiter $
create procedure sp_Chave_Busca (v_chave varchar (20))
begin 

if (v_chave != '') then
select codigoproduto from produto where codigoproduto = 'v_chave';
else
select 'CODIGO INVALIDO' as mensagem;
end if;
end $

CALL sp_Chave_Busca ('pa');
